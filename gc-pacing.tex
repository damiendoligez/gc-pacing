...
%% \begin{gather*}
%% \dot{J} \geq (1+e')S \\
%% \dot{J} \geq (L+F+G)\frac{e+1}{s + e's'} \\
%% s + e's' >= (1+e')(L+F+G)/\dot{J}
%% \end{gather*}

%% We can compute the total size of active memory:
%% \begin{equation}
%% \dot{A} = A + A' = L + L' + \dot{J} + 2\sigma\frac{1+e'}{s + e's'}
%% \end{equation}
...
%% evaluated between $e'=0$ and $e'=\dot{J}/S-1$. Its derivative is
%% positive, so it is increasing on the interval, going from $1/s$ to
%% $1/(s-1 + S/\dot{J})$. This last bound is less than or equal to
%% $1/(s-1)$, thus $f(e')$ stays between $1/s$ and $1/(s-1)$ and we get:
%% \begin{equation}
%% \dot{O} \leq \dot{J} + 2\sigma L / (s-1)
%% \end{equation}
...
%% and
%% \begin{equation}
%% \dot{O} \leq (2\sigma+1)\frac{s}{s-1}\dot{J}
%% \end{equation}
